...
func (r *PlatformParamRepository) ensureBuiltinParams(ctx context.Context) error {
	...
	items := []domain.PlatformParamDict{
		...
		},
		{
			...
			ParamKey:      "repo_url",
			Name:          "Git 仓库地址",
			Description:   "应用基础信息中的 Git 仓库地址；发布模板和 CD Jenkins 参数可直接引用，不从发布输入或 CI/CD 日志取值。",
			ParamType:     domain.ParamTypeString,
			Required:      false,
			GitOpsLocator: false,
			CDSelfFill:    false,
		...
		},
		{
			...
			ParamKey:      "env_code",
			Name:          "发布环境",
			Description:   "发布单选择的环境编码；发布模板参数、GitOps 替换规则和 Hook 变量可直接引用，例如 dev、test、prod。",
			ParamType:     domain.ParamTypeString,
			Required:      false,
			GitOpsLocator: false,
			CDSelfFill:    false,
		...
		},
		{
			...
			ParamKey:      "ci_job",
			Name:          "CI Job",
			Description:   "平台在 Jenkins CI 成功后自动注入上游 Job 全路径；CD Jenkins 参数名使用 CI_JOB，供 Copy Artifact 定位上游任务。",
			ParamType:     domain.ParamTypeString,
			Required:      false,
			GitOpsLocator: false,
			CDSelfFill:    false,
		...
		},
		{
			...
			ParamKey:      "ci_build",
			Name:          "CI Build",
			Description:   "平台在 Jenkins CI 成功后自动注入上游构建号；CD Jenkins 参数名使用 CI_BUILD，供 Copy Artifact 选择指定构建。",
			ParamType:     domain.ParamTypeString,
			Required:      false,
			GitOpsLocator: false,
			CDSelfFill:    false,

...
const activeMenuKey = computed(() => {
  ...
  if (route.path.startsWith('/components/credentials')) {
    return ['git-credential-management']
  }
  ...
  if (route.path.startsWith('/release-approval-flows')) {
    return ['release-approval-flows']
  }
...
const canViewGitCredential = computed(
  () =>
    authStore.hasPermission('component.credential.view') ||
    authStore.hasPermission('component.credential.manage'),
)
...
function goToGitCredentialManagement() {
  void router.push('/components/credentials')
}
...
function goToReleaseApprovalFlows() {
  void router.push('/release-approval-flows')
}
    ...
    >
      ...
      >
        ...
        <a-sub-menu v-if="showReleaseMenu" key="release-management">
          ...
          <a-menu-item v-if="canManageReleaseTemplate" key="release-approval-flows" @click="goToReleaseApprovalFlows">
            审批流管理
          </a-menu-item>
        </a-sub-menu>
        ...
        <a-sub-menu v-if="showComponentMenu" key="component-management">
          ...
          <a-menu-item v-if="canViewGitCredential" key="git-credential-management" @click="goToGitCredentialManagement">
            凭证管理
          </a-menu-item>
